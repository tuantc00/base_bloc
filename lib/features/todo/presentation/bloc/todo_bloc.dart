...
typedef Emit = Emitter<GenericBlocState<TodoEntity>>;

class TodoBloc extends Bloc<TodoEvent, GenericBlocState<TodoEntity>>
    with BlocHelper<TodoEntity> {
  TodoBloc({
  ...
  Future<void> getTodos(TodoFetched event, Emit emit) async {
    await getItems(
        getTodoUseCase
            .call(GetTodoParams(userId: event.userId, status: event.status)),
        emit);
  }

  Future<void> createTodo(TodoCreated event, Emit emit) async {
    await createItem(
        createTodoUseCase.call(CreateTodoParams(event.todo)), emit);
  }

  Future<void> updateTodo(TodoUpdated event, Emit emit) async {
    await updateItem(
        updateTodoUseCase.call(UpdateTodoParams(event.todo)), emit);
  }

  Future<void> deleteTodo(TodoDeleted event, Emit emit) async {
    await deleteItem(
        deleteTodoUseCase.call(DeleteTodoParams(event.todo)), emit);
  }
}
